...
def run_ista(X, y, error_function, args):
    ...
    lambdas, learning_rates = np.linspace(0.01, 1.5, 15), np.linspace(0.001, 0.3, 15)

    best_coefficients, best_time, best_error = None, None, float('inf')
    for lambda_ in lambdas:
        for learning_rate in learning_rates:
            coefficients, times = ista(X, y, lambda_, learning_rate, n_coefficients, max_time, max_steps)
...
def ista(X, y, lambda_, learning_rate, n_coefficients, max_time, max_steps):
